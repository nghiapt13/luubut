const dc = {
    headCard: {
        title: "Lưu bút của NghiaPT (Obi)",
        subheader: "Created and deployed by @yun.khngn",
        image: "https://i.pinimg.com/564x/69/63/fb/6963fbc059159e1eb19fba05f9277692.jpg",
        content: "Hello mọi người, vậy là tớ sắp ra trường. Hoặc có thể lúc tớ làm xong cái này, tớ ra trường moẹ rồi. Thì tớ cũng chẳng định làm lưu bút đâu vì tớ lười. Nhưng mà một hôm tớ thấy mọi người đăng lắm quá, thôi thì làm một cái cũng ksao. Mà cái này đặc biệt hơn, bởi tớ k dùng google form hay gì hết hè hè.",
        content2: "Đây sẽ là project cuối cùng của tớ và cũng là project tớ dùng cho mai sau này nữa =)))) chẳng biết nữa nma nếu tớ có giàu sau này, thì sự hiện diện của mọi ngừi ở hiện tại đều đáng quý cả hehe.",
        button1: "Bắt đầu thui",
        button2: "Đọc lại thư của cậu",
    },
    nameInput: {
        title: "Tên của cậu là gì nè?",
        subtitle: "Hên xui gặp nhau cho kẹo kiki 🍭.",
        nameError: "*Điền tên cậu vào iii",
        placeholder: "Tên cậu nè...",
    },
    aboutMe: {
        title: "Cậu có cảm nhân gì về tớ khong?",
        subtitle: "Suốt thời gian được tiếp xúc, làm việc,... cậu thấy thế nào về tớ 🐰.",
        aboutError: "*Nói gì iii",
        placeholder: "Tớ nghĩ cậu...",
    },
    handsome: {
        title: "Cậu đánh giá tớ thế nào?",
        subtitle: "Chấm trên thang điểm 100 của cậu nè 🫠 bít để còn cải thiện hơnn.",
        label: {
            one: "Tồi",
            fouth: "Hong bit",
            sixth: "Ok",
            full: "Đỉnh",
        }
    },
    memories: {
        title: "Chúng mình có kỉ niệm gì với nhau khong?",
        subtitle: "Nhiều hoặc ít, tớ k biết, nma kỉ niệm cũng tuyệt mà nhỉ 😺.",
        memoriesError: "*Hong có gì àaa",
        placeholder: "Kỉ niệm nè...",
    },
    message: {
        title: "Cậu có muốn nói gì với tớ khong?",
        subtitle: "Một lời cảm ơn chẳng hạn, hoặc trách tớ kọn ceo cũng được 🗿.",
        messageError: "Hong gửi gì àaa",
        placeholder: "Gửi di nèee...",
    },
    submit: {
        title: "Vậy là xong ròi nèee!!!",
        subheader: "Cảm ơn cậu nhaaaaa",
        image: "https://wallpapers.com/images/featured/vmdj9lfm2un8rji3.jpg",
        content: "Cảm ơn cậu nhíeee, ít nhiều thì cũng từng, hoặc chưa từng gặp nhau nhiều, cũng ksao. Bởi cấp 3 của tớ chán òm (k hẳn là chán vì tớ cũng đỉnh chứ nhỉ), nên sự hiện diện của cậu cũng tuyệt vời đóoo.",
        content2:"Còn một bước nữa thôi nè là xong ròi. Site tớ dùng để gửi form nên để tránh spam, cậu xác nhận captcha bên dưới nè. Biết đâu lại là robot 🤖.",
        button: "Gửi cho tớ lunnn",
    },
}

export default dc;